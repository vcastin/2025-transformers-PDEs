...
inits = init_grid(10, 10, 1)
S_lists_MH = [
    evolution(S, Q, K, V, rhs=rhs_multihead, step_size=0.3, tol_low=1e-5, H=2)[0]
    for S in inits
]
inits = init_grid(10, 10, 1)
S_lists_sink = [
    evolution(S, Q, K, V, rhs=rhs_sinkhorn, step_size=0.5, tol_low=1e-5)[0]
    for S in inits
]

plot_evolution(
    "trad_neg_def_V.pdf", S_lists, elev=20, azim=-174, starting_point=True, save=True
)
plot_evolution(
    "L2_neg_def_V.pdf", S_lists_L2, elev=20, azim=-174, starting_point=True, save=True
)
plot_evolution(
    "MH_neg_def_V.pdf", S_lists_MH, elev=20, azim=-174, starting_point=True, save=True
)
plot_evolution(
    "sink_convergence.pdf",
    S_lists_sink,
    elev=20,
    azim=-174,
    starting_point=True,
...
inits = init_grid(10, 10, 1)
S_lists = [
    evolution(S, Q, K, V=np.eye(2), step_size=0.5, tol_low=1e-4)[0] for S in inits
]
inits = init_grid(10, 10, 1)
S_lists_L2 = [
    evolution(S, Q, K, V=np.eye(2), rhs=rhs_L2, step_size=0.5, tol_low=1e-4)[0]
    for S in inits
]
...
S_lists_MH = [
    evolution(
        S, Q, K, V=np.eye(2), rhs=rhs_multihead, step_size=0.5, tol_low=1e-4, H=2
    )[0]
    for S in inits
]
plot_evolution(
...
inits = init_grid(20, 20, 1, limits=[-1.0, 1.0, -1.0, 1.0])
S_lists_V = [evolution(S, Q, K, V, step_size=0.01, tol_low=1e-7)[0] for S in inits]
inits = init_grid(20, 20, 1, limits=[-1.0, 1.0, -1.0, 1.0])
S_lists_V = [
    evolution(S, Q, K, V, rhs=rhs_L2, step_size=0.01, tol_low=1e-7)[0] for S in inits
]

plot_evolution(
    "trad_mixed_3d.pdf", S_lists_V, azim=-88, elev=20, starting_point=False, save=True
)
plot_evolution(
    "L2_mixed_3d.pdf", S_lists_V, azim=-88, elev=20, starting_point=False, save=True
)
...
inits = init_grid(40, 40, 1, limits=[-1.0, 1.0, -1.0, 1.0])
S_lists = [
    evolution(
        S,
        Q,
...
S_lists_MH_renorm = [
    np.array([S / np.trace(S) for S in S_list]) for S_list in S_lists_MH
]

inits = init_grid(40, 40, 1, limits=[-1.0, 1.0, -1.0, 1.0])
S_lists_L2 = [
    evolution(
        S, Q, K, V=np.eye(2), rhs=rhs_L2, step_size=0.001, tol_low=1e-5, tol_high=1e4
    )[0]
    for S in inits
]
S_lists_L2_renorm = [
    np.array([S / np.trace(S) for S in S_list]) for S_list in S_lists_L2
]
...
S_lists_sink_renorm = [
    np.array([S / np.trace(S) for S in S_list]) for S_list in S_lists_sink
]
...
inits = init_grid(30, 30, 1, limits=[-1.0, 1.0, -1.0, 1.0])
...
inits = init_grid(30, 30, 1, limits=[-1.0, 1.0, -1.0, 1.0])
S_lists_V_L2 = [
    evolution(S, Q, K, V, rhs=rhs_L2, step_size=0.002, tol_low=1e-4)[0] for S in inits
]
S_lists_renorm_V_L2 = [
    np.array([S / np.trace(S) for S in S_list]) for S_list in S_lists_V_L2
]

inits = init_grid(30, 30, 1, limits=[-1.0, 1.0, -1.0, 1.0])
S_lists_V_MH = [
    evolution(S, Q, K, V, rhs=rhs_multihead, step_size=0.002, tol_low=1e-4)[0]
    for S in inits
]
S_lists_renorm_V_MH = [
    np.array([S / np.trace(S) for S in S_list]) for S_list in S_lists_V_MH
]
