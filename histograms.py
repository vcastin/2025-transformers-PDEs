...
# rank_lists_id, exceptions = create_histograms(dimensions, Q_list, K_list, [np.eye(d) for d in dim_repeated], rhs_trad, n_inits, n_per_dim, step_size_list=step_size_list, tol_evol=1e-4, max_iter_evol=60000, tol_rank=1e-1)
...
# rank_lists_V, exceptions = create_histograms(dimensions, Q_list, K_list, V_list, rhs_trad, n_inits, n_per_dim, step_size_list=step_size_list, tol_evol=5e-5, max_iter_evol=90000, tol_rank=1e-1, n_max=100*n_per_dim)
...
rank_lists_id, exceptions = create_histograms(dimensions, Q_list, K_list, [np.eye(d) for d in dim_repeated], rhs_L2, n_inits, n_per_dim, step_size_list=step_size_list, tol_evol=1e-4, max_iter_evol=60000, tol_rank=1e-1)
...
V_list = [np.random.randn(d, d) for d in dim_repeated]
V_list = [np.dot(V.T, V) for V in V_list]
rank_lists_V, exceptions = create_histograms(dimensions, Q_list, K_list, V_list, rhs_L2, n_inits, n_per_dim, step_size_list=step_size_list, tol_evol=5e-5, max_iter_evol=50000, tol_rank=1e-1, n_max=1000)
